...
from hummingbot.market.dranite.dranite_market import DraniteMarket
...
class DraniteMarketUnitTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.market: DraniteMarket = DraniteMarket(
            conf.dranite_api_key,
            conf.dranite_secret_key,
            symbols=["ltc-btc", "ltc-eth"]
        )
        print("Initializing Dranite Pro market... this will take about a minute.")
    ...
    def setUp(self):
        self.db_path: str = realpath(join(__file__, "../dranite_test.sqlite"))
        try:
    ...
    def test_get_fee(self):
        limit_fee: TradeFee = self.market.get_fee("ltc", "btc", OrderType.LIMIT, TradeType.BUY, 1, 1)
    ...
    def test_limit_buy(self):
        self.assertGreater(self.market.get_balance("ltc"), Decimal("0.1"))
        symbol = "ltc-eth"
    ...
    # NOTE that orders of non-USD pairs (including btc pairs) are LIMIT only
    ...
    def test_cancel_order(self):
        ...
        self.assertGreater(self.market.get_balance("ltc"), amount)
    ...
    def test_deposit_info(self):
        [deposit_info] = self.run_parallel(
            self.market.get_deposit_info("ltc")
        )
    ...
    def test_withdraw(self):
        ...
        self.assertGreaterEqual(self.market.get_balance("ltc"), Decimal('1'))

        # Withdraw ltc from Dranite to test wallet.
        self.market.withdraw(self.wallet.address, "ltc", Decimal('1'))
        ...
        self.assertEqual("ltc", withdraw_asset_event.asset_name)
    ...
    def test_orders_saving_and_restoration(self):
        ...
        try:
            ...
            # Try to put limit buy order for 0.04 ltc, and watch for order creation event.
            current_bid_price: Decimal = self.market.get_price(symbol, True)
            ...
            for event_tag in self.events:
                self.market.remove_listener(event_tag, self.market_logger)
            self.market: DraniteMarket = DraniteMarket(
                dranite_api_key=conf.dranite_api_key,
                dranite_secret_key=conf.dranite_secret_key,
                symbols=["ltc-btc", "ltc-eth"]
            )
            for event_tag in self.events:
                self.market.add_listener(event_tag, self.market_logger)
    ...
    def test_order_fill_record(self):
        ...
        try:
            # Try to buy 0.04 ltc from the exchange, and watch for completion event.
            amount: Decimal = Decimal("0.04")
