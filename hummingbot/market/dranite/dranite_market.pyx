...
from hummingbot.market.dranite.dranite_order_book_tracker import DraniteOrderBookTracker
...
cdef class DraniteMarketTransactionTracker(TransactionTracker):
    cdef:
        DraniteMarket _owner

    def __init__(self, owner: DraniteMarket):
        super().__init__()
        self._owner = owner
...
cdef class DraniteMarket(MarketBase):
    ...
    def __init__(self,
        ...
        self._dranite_auth = DraniteAuth(dranite_api_key, dranite_secret_key)
        self._order_book_tracker = DraniteOrderBookTracker(data_source_type=order_book_tracker_data_source_type,
                                                           symbols=symbols)
        self._user_stream_tracker = DraniteUserStreamTracker(dranite_auth=self._dranite_auth,
                                                             symbols=symbols)
        ...
        self._tx_tracker = DraniteMarketTransactionTracker(self)
        self._trading_rules = {}
    ...
    @property
    def dranite_auth(self) -> DraniteAuth:
        """
        :return: DraniteAuth class (This is unique to Dranite market).
        Read more here: https://www.dranite.com/
        """
        return self._dranite_auth
    ...
    def restore_tracking_states(self, saved_states: Dict[str, any]):
        ...
        *required
        ...
        self._in_flight_orders.update({
            key: DraniteInFlightOrder.from_json(value)
    ...
    async def get_active_exchange_markets(self) -> pd.DataFrame:
        ...
        *required
        ...
        """
        return await DraniteAPIOrderBookDataSource.get_active_exchange_markets()
    ...
    async def check_network(self) -> NetworkStatus:
        ...
        *required
        ...
        """
        try:
            await self._api_request("get", path_url="/auth")
    ...
    async def _api_request(self,
        ...
        """
        A wrapper for submitting API requests to Dranite
        :returns: json data from the endpoints
        """
        assert path_url is not None or url is not None
        if(".json" in path_url):
            url = f"{self.DRANITE_API_ENDPOINT_TEMP}{path_url}" if url is None else url
        else:
            url = f"{self.DRANITE_API_ENDPOINT}{path_url}" if url is None else url
        data_str = "" if data is None else json.dumps(data)
        headers = self.dranite_auth.get_headers(path_url)
        ...
        for tracked_order in tracked_orders:
            ...
            if order_update is None:
                self.logger().network(
                    ...
                    app_warning_msg=f"Could not fetch updates for the order {tracked_order.client_order_id}. "
                                    f"Check Authentication key and network connection."
                )
                continue
            ...
            if execute_amount_diff > s_decimal_0:
                order_filled_event = OrderFilledEvent(
                    ...
                    # Dranite's websocket stream tags events with order_id rather than trade_id
        ...
        async for event_message in self._iter_user_event_queue():
            try:
                ...
                if content.get("reason") == "filled":  # Only handles orders with "done" status
                    if tracked_order.trade_type == TradeType.BUY:
                        self.logger().info(f"The market buy order {tracked_order.client_order_id} has completed "
                                           f"according to Dranite user stream.")
                        self.c_trigger_event(self.MARKET_BUY_ORDER_COMPLETED_EVENT_TAG,
                                             BuyOrderCompletedEvent(self._current_timestamp,
                                                                    tracked_order.client_order_id,
                                                                    tracked_order.base_asset,
    ...
    async def place_order(self, order_id: str, symbol: str, amount: Decimal, is_buy: bool, order_type: OrderType,
        ...
        # path_url = "/api/v1/order/create" #production
        data = {
            "symbol": symbol,
            "amount": float(amount),
            "price": float(price),
    ...
    async def execute_buy(self,
        ...
        try:
            ...
            self.logger().network(
                f"Error submitting buy {order_type_str} order to Dranite for "
                f"{decimal_amount} {symbol} {price}.",
                exc_info=True,
                app_warning_msg="Failed to submit buy order to Dranite. "
                                "Check Authentication Headers and network connection."
            )
    ...
    async def execute_sell(self,
        ...
        try:
            ...
            order_type_str = "MARKET" if order_type == OrderType.MARKET else "LIMIT"
            self.logger().network(
                f"Error submitting sell {order_type_str} order to Dranite for "
                f"{decimal_amount} {symbol} {price}.",
                exc_info=True,
                app_warning_msg="Failed to submit sell order to Dranite. "
    ...
    async def execute_cancel(self, symbol: str, order_id: str):
        ...
        try:
            ...
            # path_url = f"/order/cancel/{exchange_order_id}" # production
            ...
            if "order not found" in e.message:
                ...
                self.logger().info(f"The order {order_id} does not exist on Dranite. No cancellation needed.")
                self.c_stop_tracking_order(order_id)
                self.c_trigger_event(self.MARKET_ORDER_CANCELLED_EVENT_TAG,
                                     OrderCancelledEvent(self._current_timestamp, order_id))
                return order_id
            ...
            self.logger().network(
                ...
                exc_info=True,
                app_warning_msg=f"Failed to cancel the order {order_id} on Dranite. "
                                f"Check Authentication keys and network connection."
            )
        return None
        ...
        *required
        ...
        try:
            ...
            self.logger().network(
                ...
                exc_info=True,
                app_warning_msg="Failed to cancel order on Dranite. Check API key and network connection."
            )
        ...
        while True:
            try:
                ...
                self.logger().network(
                    ...
                    exc_info=True,
                    app_warning_msg=f"Could not fetch account updates on Dranite. "
                                    f"Check Authentication key and network connection."
                )
        ...
        while True:
            try:
                ...
                self.logger().network(
                    ...
                    exc_info=True,
                    app_warning_msg=f"Could not fetch trading rule updates on Dranite. "
    ...
    cdef c_start_tracking_order(self,
        ...
        """
        self._in_flight_orders[client_order_id] = DraniteInFlightOrder(
            client_order_id,
            None,
            symbol,
            order_type,
    ...
    cdef object c_get_order_size_quantum(self, str symbol, object order_size):
        ...
        *required
        ...
        # Dranite is using the min_order_size as max_precision
